...
class TrueOBS:
    '''
    Класс предназначен для оптимизации весов нейронной сети после удаления (обнуления) некоторых из них.

    Атрибуты класса:
    - layer (torch.Tensor) : слой нейронной сети, для которого будет применяться квантование
    - dev (torch.device) : устройство, на котором находятся веса слоя (e.g. 'cpu', 'cuda')
    - rows (int) : количество строк весов
    - columns (int) : количество столбцов весов
    - H (torch.Tensor) : матрица Гессе, которая накапливается в процессе обучения
    - nsamples (int) : количество обработанных батчей
    - rel_damp (float) : коэффициент затухания, используемый для стабилизации матрицы Гессе
    '''

    def __init__(self, layer, rel_damp=0):
        '''
        Инициализирует объект TrueOBS для заданного слоя нейронной сети.
        '''
        self.layer = layer
    ...
    def add_batch(self, inp, out):
        '''
        Добавляет батч данных для накопления статистики матрицы Гессе.

        Аргументы:
        - inp (torch.Tensor) : входные данные батча
        - out (torch.Tensor) : выходные данные батча
        '''
        if DEBUG:
            self.inp1 = inp
            self.out1 = out
    ...
    def invert(self, H):
        '''
        Инвертирует матрицу Гессе с использованием разложения Холецкого.

        Аргументы:
        - H (torch.Tensor) : матрица Гессе

        Возвращает:
        - инвертированную матрицу Гессе
        '''
        try:
    ...
    def prepare(self, columnslast=False):
        '''
        Подготавливает данные для квантизации, включая матрицу Гессе и веса слоя.

        Аргументы:
        - columnslast (bool) : флаг, указывающий, должны ли столбцы быть последними

        Возвращает:
        - веса слоя
        - матрицу Гессе
        - инвертированную матрицу Гессе
        - матрицу для хранения ошибок
        '''
        if columnslast: 
    ...
    def prepare_iter(self, i1, parallel, W, Hinv1):
        '''
        Подготавливает данные для итерации по строкам весов.

        Аргументы:
        - i1 (int) : начальный индекс строки
        - parallel (int) : количество строк, обрабатываемых параллельно
        - W (torch.Tensor) : веса слоя
        - Hinv1 (torch.Tensor) : инвертированная матрица Гессе

        Возвращает:
        - конечный индекс строки
        - количество строк для обработки
        - веса для обработки
        - инвертированную матрицу Гессе для обработки
        - маску для обнуленных весов
        - диапазон индексов для индексации внутри текущей параллельной обработки строк
        - диапазон индексов для обновления весов в исходном массиве весов W
        '''
    ...
    def prepare_sparse(self, w, mask, Hinv, H): 
        '''
        Подготавливает данные для разреженных весов, обновляя инвертированную матрицу Гессе.

        Аргументы:
        - w (torch.Tensor) : веса
        - mask (torch.Tensor) : маска для обнуленных весов
        - Hinv (torch.Tensor) : инвертированная матрица Гессе
        - H (torch.Tensor) : матрица Гессе

        Возвращает:
        - 1 + минимальное количество обнуленных весов среди всех строк w
        '''
    ...
    def quantize(self, parallel=32):
        '''
        Выполняет квантизацию весов слоя, минимизируя ошибку квантования.
    ...
    def nmprune(self, n=2, m=4, parallel=32):
        '''
        Удаляет веса слоя, минимизируя ошибку квантования.

        Аргументы:
        - n (int) : количество весов в блоке 
        - m (int) : размер блока
        - parallel (int) : количество строк, обрабатываемых параллельно
        '''
    ...
    def prepare_unstr(self, parallel=32):
        '''
        Подготавливает данные для неструктурированного удаления весов.
    ...
    def prune_unstr(self, sparsities):
        '''
        Удаляет неструктурированные веса, минимизируя ошибку квантования.

        Аргументы:
        - sparsities (list) : разреженности слоев
        '''
        return self.prune_blocked(sparsities)

    def prepare_blocked(self, size=4, parallel=32):
        '''
        Подготавливает данные для блочного удаления весов.
    ...
    def prune_blocked(self, sparsities):
        '''
        Удаляет блочные веса, минимизируя ошибку квантизации.
        ...
        self.loss_ans = {sp: l for sp, l in zip(sparsities, losses)}
